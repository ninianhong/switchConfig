...
std::vector<VertexNode>::const_iterator it;
...
void init_vexMap0_bclk( void )
	...
	void init_vexMap1_bclk( void )
	{
		...
		vexMap1_bclk["T"] = temp;
		temp.clear();
		temp.insert( {"T[30.1]",5});
		vexMap1_bclk["U"] = temp;
		temp.clear();
		temp.insert( {"T[30.2]",5});
		vexMap1_bclk["V"] = temp;
		temp.clear();
		temp.insert( {"T[30.3]",5});
		vexMap1_bclk["W"] = temp;
		temp.clear();
		temp.insert( {"T[30.4]",5});
		vexMap1_bclk["X"] = temp;
		temp.clear();
		temp.insert( {"T[30.5]",5});
		vexMap1_bclk["Y"] = temp;				
	}
	#else
	void init_vexMap1_bclk( void )
